...
async def analyze_audio_route(
    ...
    try:
        ...
        try:
            ...
            # Generate quantitative linguistic analysis
            linguistic_analysis = analyze_linguistic_patterns(
                transcript_text, 
                audio_quality_metrics.get('duration')
            )
            ...
            validated_gemini_analysis = validate_and_structure_gemini_response(gemini_raw_response, transcript_text, linguistic_analysis)
            ...
            logger.warning(f"Speech recognition: Could not understand audio from file {audio.filename or 'unnamed'} in session {current_session_id}.")
            ...
            logger.error(f"Speech recognition service error for file {audio.filename or 'unnamed'} in session {current_session_id}: {e}")

...
def query_gemini_with_audio(audio_path: str, transcript: str, flags: Dict[str, Any], session_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        - credibility_score must be integer 0-100 based on vocal and content analysis
...
def validate_and_structure_gemini_response(raw_response: Dict[str, Any], transcript: str, quantitative_linguistic: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Validate and fix gemini_summary (handle list-to-string conversion)
    ...
    if not isinstance(validated_response.get('gemini_summary'), dict):
        ...
        for summary_field in required_summary_fields:
            if summary_field not in validated_response['gemini_summary'] or not validated_response['gemini_summary'][summary_field]:
                ...
                # Convert lists to strings if needed (common issue with Gemini responses)
                field_value = validated_response['gemini_summary'][summary_field]
                if isinstance(field_value, list):
                    validated_response['gemini_summary'][summary_field] = '; '.join(str(item) for item in field_value)
                    logger.info(f"Converted list to string for {summary_field}: {field_value}")
                elif not isinstance(field_value, str):
                    validated_response['gemini_summary'][summary_field] = str(field_value)

    # Validate linguistic_analysis

...
async def analyze_audio(
    ...
    try:
        ...
        logger.info(f"Received audio file: {audio.filename}, Content-Type: {audio.content_type}, Size: {file_size} bytes")
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(audio.filename)[1] or ".tmp") as temp_audio:
        ...
        try:
            ...
            with sr.AudioFile(wav_path) as source:
                ...
                try:
                    ...
                    logger.info(f"Transcript generated: \"{transcript[:100]}...\"") # Log snippet
                except sr.UnknownValueError:
                    logger.warning(f"Speech recognition: Could not understand audio from file {audio.filename}.")
                    ...
                    logger.error(f"Speech recognition service error for file {audio.filename}: {e}")
            ...
            if not transcript or len(transcript.strip()) < 10: # Check after attempting transcription
                logger.warning(f"Transcript too short or empty for file {audio.filename}: '{transcript}'")
